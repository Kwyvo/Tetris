...
class CustomCanvas:
    ...
    def __init__(
        ...
        resizing: bool = False,
        couleur_fond: str = "black",
        title: str = "Fenêtre Tetris",
        icon: Optional[str] = None  # Chemin de l'icône en option
    ) -> None:
        self.width = width
        self.height = height
        self.interval = 1 / refresh_rate

        # root Tk object avec titre
        self.root = tk.Tk()
        self.root.title(title)

        # Charger et appliquer l'icône si elle est fournie
        if icon:
            try:
                icon_image = PhotoImage(file=icon)
                self.root.iconphoto(True, icon_image)
            except Exception as e:
                print(f"Impossible de charger l'icône : {e}", file=sys.stderr)

        # Création du canvas avec couleur de fond
        ...
        # Ajout du canvas et réglage
        self.canvas.pack(fill=tk.BOTH, expand=tk.YES)
        self.root.resizable(width=resizing, height=resizing)
        self.canvas.focus_set()
        self.first_resize = True

        # Binding des événements et mise à jour initiale
...
def cree_fenetre(
    ...
    color: str = "black",
    title: str = "Fenêtre Tetris",  # Nouveau paramètre pour le titre
    icon = str
) -> None:
    global __canevas
    if __canevas is not None:
        raise FenetreDejaCree(
            'La fenêtre a déjà été créée avec la fonction "cree_fenetre".'
        )
    __canevas = CustomCanvas(
        largeur, hauteur, frequence, resizing=redimension, couleur_fond=color, title=title, icon = icon
    )
...
async def animation_gif(
    ...
    """
    Affiche une animation GIF sur le canvas.

    :param float x: abscisse du point d'ancrage
    :param float y: ordonnée du point d'ancrage
    :param str fichier: chemin du fichier GIF
    :param str ancrage: position du point d'ancrage (par défaut 'center')
    :param str tag: étiquette d'objet (défaut : pas d'étiquette)
    :param int intervalle: intervalle entre frames en ms (par défaut 100ms)
    """
    assert __canevas is not None
    if not PIL_AVAILABLE:
        raise ImportError("Pillow (PIL) est nécessaire pour afficher des GIFs animés.")

    gif = Image.open(fichier)
    frames = []
    ...
    try:
        while True:
            ...
            gif.seek(len(frames))  # Passe à la frame suivante
    except EOFError:
        pass  # Fin du fichier GIF

    # Ajouter un objet unique pour le GIF
    gif_id = __canevas.canvas.create_image(x, y, anchor=ancrage, image=frames[0], tags=tag)

    def animer(index: int = 0) -> None:
        """
        Met à jour le canvas avec la frame courante et programme la suivante.
        """
        if not frames:
            return

         # Mettre à jour l'image existante au lieu de supprimer tout
        __canevas.canvas.itemconfig(gif_id, image=frames[index])

        # Programmer la prochaine frame
        index = (index + 1) % len(frames)
        __canevas.root.after(intervalle, animer, index)
...
def texte(
    ...
    return __canevas.canvas.create_text(
        x, y, text=chaine, font=(police, taille, style), tags=tag, fill=couleur, anchor=ancrage
    )
