...
def pourrissement_jeu(hauteurFenetre,largeurFenetre,sauv_plateau = [],sauv_coord_pieces_placer = [],score= 0,compteur_ligne = 0,niveau = 1,mode_de_jeu="pourri"  ):
    """
    ...
    while not gameOver:
        ...
        dernier_temps_disparition = time()
        ...
        while i_debut <= i_fin - taille_piece: # hauteur du plateau moins l'espace de fin pour la pièce
            ...
            #--------------------------MODE POURISSEMENT---------------------------------------
            if time() - dernier_temps_disparition >= float(disparition_bloque) and coord_pieces_placer:
                c = randint(0,len(coord_pieces_placer)-1)
                a = randint(0,len(coord_pieces_placer[c])-1)
                plateau[coord_pieces_placer[c][a][0]][coord_pieces_placer[c][a][1]] = 0
                coord_pieces_placer[c].remove(coord_pieces_placer[c][a])
                dernier_temps_disparition = time()
        ...
        if [] in coord_pieces_placer:
            coord_pieces_placer.remove([])
        #-----------------------------------------------------------------------------------------------------------
